...
export async function GET(
  ...
  try {
    result = await db.execute({
      sql: "SELECT id, guid, url, title, content, author, published_at, is_read, created_at, image_url FROM articles WHERE feed_id = ? ORDER BY published_at DESC, id DESC",
      args: [feedId],
    ...
    result = await db.execute({
      sql: "SELECT id, guid, url, title, content, author, published_at, is_read, created_at FROM articles WHERE feed_id = ? ORDER BY published_at DESC, id DESC",
      args: [feedId],
    }) as { rows: Array<Record<string, unknown>> };
  }
